...
We note that the linear approximation seems to hold. 
...
We continue by investigating the third chain ladder assumption. According to Mack's article it is suggested to plot three different plots of residuals. First we plot $C_{i,k+1} - C_{i,k}f_{k0}$ against $C_{i,k}$, where $f_{k0} = \sum_{i=1}^{I-k} C_{i,k} C_{i,k+1} / \sum_{i=1}^{I-k} C_{i,k}^2$. 
...
Then we plot $(C_{i,k+1} - C_{i,k}f_{k1}) / \sqrt{C_{i,k}}$ against $C_{i,k}$, where $f_{k1} = \sum_{i=1}^{I-k} C_{i,k+1} / \sum_{i=1}^{I-k} C_{i,k}$. 
...
Lastly, we plot $(C_{i,k+1} - C_{i,k}f_{k2}) / C_{i,k}$ against $C_{i,k}$, where $f_{k2} = \frac{1}{I-k}\sum_{i=1}^{I-k} ( C_{i,k+1} / C_{i,k})$. 
...
Mack suggest that all this should be done for every developement year with at least $6$ data points, i.e for $k \leq I - 6$, however we chose do only include plots for $k=1$ since we could not see any systematic rasiduals or non-random pattern for any developement years. We can therefore conclude that the third chain-ladder assumption is satisfied.
