...
#grid.arrange(resFrame1, resFrame2)
...
#MAX GÖR FÖRDELNINGSHISTROGRAM FÖR ÅRENS RANKING FÖR BÅDA BRANCHES!!
...
$$
Var(R) = \sum_{i=2}^{I}Var(C_{i,I}) + C_{i,I}\left(\sum_{j=i+1}^I C_{j,I}\right)\sum_{k=I+1-i}^{I-1}\frac{2\sigma_k^2}{f_k^2\sum_{n=1}^{I-k}C_{n,k}}
$$
...
riskCalculation <- function(ct, ctAlt){
  ...
  for(i in 2:n){
    ...
    sum = sum + C[i,n]*sum(C[(i+1):n,n])*2*sum((sigmaHat/fHat^2)[(n+1-i):(n-1)])/sum(C[1:(n-k), k])
...
#riskCalculation(ct1, ct1Alt)

```

By doing this we get the following ERROR :((
