...
Since the claims triangle is limited to 10 development years the data has been truncated to accomodate this limit. The final 10 years of the data can be aggregated in order to create the following two paid claims triangles. 
...
kable(rbind(c("Product 1" , sum(reserve1)), c("Product 2", sum(reserve2))), digits = 2, caption = "Total Chain Ladder Reserve") %>%  kable_styling(latex_options =c("hold_position"))
...
We begin by examing whether or not the we have an approximate linear relationship between $C_{i,k}$ and $C_{i,k+1}$ for $i = 1,...,10$ for the two branches.
...
Mack suggest that all this should be done for every developement year with at least $6$ data points, i.e for $k \leq I - 6$, however we chose do only include plots for $k=1$ since we could not see any systematic residuals or non-random pattern for any developement years. We can therefore conclude that the third chain-ladder assumption is satisfied.

Lastly we want to examine whether or not we have any calender year effects, the second chain-ladder assumption. To do this we follow the procedure explained in Appendix H in Mack's article. We begin by creating a triangle for the developement factors which we can see in Table 6 for branch 1 and in Table 9 for branch 2. We continue by computing the column means in this triangle. Now, if the developement factor for a cell is
...
For branch 1 we got $Z=$ `r teststatistic1` which is inside the corresponding confidence interval (`r confidenceInt1[1]`, `r confidenceInt1[2]`). However, for branch 2 we get $Z=$ `r teststatistic2` which is slighly outside the corresponding confidence interval (`r confidenceInt2[1]`, `r confidenceInt2[2]`) and thus we can reject the null-hypothesis of no calendar year effects for branch 2. The second chain-ladder assumption can therefore be questioned for branch 2. 
