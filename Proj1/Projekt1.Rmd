...
samples1 <- data.frame(matrix(ncol = 2, nrow = 0))
...
samples1 %>% 
  ggplot(aes(x = Total)) +
    ...
    ggtitle("Fig 1: Distribution of sampled annual costs of the two branches")

```

#Exercise 5

We now want to implement two separate XL-covers. The first cover caps losses for the $10\%$ worst claims. For our two branches these cut-off $M_1$ and $M_2$ are the following
...
expectedCost <- data %>% 
  inner_join(mVals, by = "ClaimType") %>% 
  mutate(Expenditures = pmax(ClaimCost-M,0)) %>% 
  group_by(ClaimType, Year) %>% 
...
noXL <- data %>% 
  ...
  ggtitle("Historical claim costs without XL-covers (price not included)")

XL <- data %>% 
  mutate(ClaimCost = ifelse(ClaimType == 1, pmin(ClaimCost, mVals$M[1]), pmin(ClaimCost, mVals$M[2]))) %>% 
  ggplot(aes(x = ClaimCost)) +
  geom_histogram(bins = 50) +
  facet_wrap(~ClaimType, scales = "free") +
  ggtitle("Historical claim costs with XL-covers (price not included)")

#OBS någon får gärna kolla limits för facet_wrap eller bara hårdkoda

grid.arrange(noXL, XL)

```

From the figure above we see the distribution of claim costs for the two branches with, and wihtout, XL-cover. Of course these covers come at a price which we can estimate from histrocial data. 
...
We then want to examine how the purchase of these XL-covers would impact our total losses for the forthcoming year. We saw a simulated distirbution of the losses without the XL-covers in figure 1. If we implement the covers we get the following simulated distributions.

```{r echo = FALSE, cache = TRUE}

samples2 <- data.frame(matrix(ncol = 2, nrow = 0))
...
for(i in 1:5000){
  ...
  costs <- bsSample %>% 
    inner_join(mVals, by = c("Type" = "ClaimType")) %>% 
...
We see that the distribution is lighter-tailed which of course is to be expected after the purchase of an XL-cover.

#Exercise 6

We now want to implement an SL-cover instead of an XL-cover for both insurance branches. Like the XL-covers the SL-covers insure against the $10\%$ worst annual costs at a price of $120\%$ of the expected cost.
...
#OSÄKER PÅ NEDAN!!!! Samma för Exercise 7
...
expectedCost2 <- data %>% 
  ...
  inner_join(mVals2, by = c("ClaimType" = "Type")) %>% 
...
#   geom_histogram(bins = 50) +
#   facet_wrap(~ClaimType, scales = "free") +
#   ggtitle("Historical costs without SL covers (price not included)")
# 
# SL <- data %>% 
#   group_by(ClaimType, Year) %>% 
#   summarize(Total = sum(ClaimCost)) %>% 
#   mutate(Total = ifelse(ClaimType == 1, pmin(Total, mVals2$M[1]), pmin(Total, mVals2$M[2]))) %>% 
#   ggplot(aes(x = Total)) +
#   geom_histogram(bins = 50) +
#   facet_wrap(~ClaimType, scales = "free") +
#   ggtitle("Historical costs with SL covers (price not included)")

prices2 <- expectedCost2 %>% 
  group_by(ClaimType) %>% 
  summarize(Price = mean(Price)) 

samples3 <- data.frame(matrix(ncol = 2, nrow = 0))
...
for(i in 1:5000){
  ...
  costs <- bsSample %>% 
    ...
    mutate(Cost = pmin(Total, M)) %>% 
    dplyr::select(Type, Cost)
  
  costs <- (costs + prices2) %>% 
    mutate(Type = Type/2)
  
  samples3 <- rbind(samples3, costs)
...
samples1 %>% 
  ggplot(aes(x = Total)) +
    geom_histogram(bins = 20) +
    facet_wrap(~Type, scales = "free", labeller = labeller(ClaimType = c("1" = "Product 1", "2" = "Product 2"))) +
    ggtitle("Distribution of sampled annual costs of the two branches without SL-covers")

samples3 %>% 
  ggplot(aes(x = Cost)) +
    geom_histogram(bins = 20) +
    facet_wrap(~Type, scales = "free", labeller = labeller(ClaimType = c("1" = "Product 1", "2" = "Product 2"))) +
    ggtitle("Distribution of sampled annual costs of the two branches with SL-covers")
...
(samples3$Cost[samples3$Type == 1] + samples3$Cost[samples3$Type == 2]) %>% 
  data.frame() %>% 
  setNames("Cost") %>% 
  ggplot(aes(x = Cost)) +
  geom_histogram(bins = 30) +
...
for(i in 1:5000){
  ...
  cost = min(sum(bsSample$Cost), mVals3)
  
  samples4 <- c(samples4, cost + prices3)
  
}

```

The cost of this cover, based on historical data, is `r prices3`
