...
samples1 %>% 
  ggplot(aes(x = Total)) +
    ...
    ggtitle("Figure 1: Distribution of sampled annual costs of the two branches")
...
copulaSampler <- function(rho){
  ...
  for(i in 1:365){
    ...
    x <- rnorm(1)
    y <- rho[floor(i/((366/12))) + 1]*x + sqrt(1-rho[floor(i/(366/12)) + 1]^2)*x
...
copulaSampler(rep(0,12))


```

From Figure 1 above we note that the distributions look fairly normal with branch $2$ having both a higher expectation and variance compared to branch $1$. 
...
samples2 %>% 
  ggplot(aes(x = Total)) +
    geom_histogram(bins = 20) +
    facet_wrap(~Type, scales = "free", labeller = labeller(ClaimType = c("1" = "Product 1", "2" = "Product 2"))) +
    ggtitle("Figure 2: Distribution of sampled annual costs of the two branches with XL-covers")
...
samples3 %>% 
  ggplot(aes(x = Total, y = ..density..)) +
    geom_histogram() +
    facet_wrap(~Type, scales = "free", labeller = labeller(ClaimType = c("1" = "Product 1", "2" = "Product 2"))) +
    ggtitle("Figure 3: Sampled annual costs with SL-covers")
