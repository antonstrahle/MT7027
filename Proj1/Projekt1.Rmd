...
#  # filter(ClaimType == 2) %>% 
#   ggplot(aes(x= Year, y = log(ClaimCost))) +
#   geom_boxplot() +
#   facet_wrap(~ ClaimType, labeller = labeller(ClaimType = c("1" = "Branch 1","2" = "Branch 2"))) +
#   ggtitle("Boxplot of log of claim cost vs month")

# plotClaimCostM1 <- data %>% 
...
#     geom_point() +
#     ylab("Mean Claim") +
#     ggtitle("Average claim cost for each \nmonth for branch 1")
# 
# plotClaimCostM2 <- data %>% 
#   group_by(ClaimType, Month)  %>% 
#   summarize(MeanClaim = mean(ClaimCost)) %>% 
#   filter(ClaimType == 2) %>% 
#   ggplot(aes(x = Month, y = MeanClaim)) +
...
#     ggtitle("Average claim cost for each \nmonth for branch 2")
# 
# grid.arrange(plotClaimCostM1, plotClaimCostM2, nrow = 1)
...
```{r echo = FALSE ,warning = FALSE, message = FALSE}
# aggregatedData %>% 
#   group_by(ClaimDay, ClaimType) %>% 
#   summarize(Mean = mean(NumberOfClaims)) %>% 
#   ggplot(aes(x=ClaimDay, y = Mean)) +
#   geom_point()
#   head()
# 
# data %>% 
#   mutate(ClaimDay = as.numeric(ClaimDay)) %>% 
#  # filter(ClaimType == 2) %>% 
#   ggplot() +
#   geom_point(aes(x= ClaimDay, y = log(ClaimCost)), alpha= 0.05) +
#   facet_wrap(~ ClaimType, labeller = labeller(ClaimType = c("1" = "Branch 1","2" = "Branch 2"))) +
#   ggtitle("Boxplot of log of claim cost vs month")
...
hist(log((data %>% 
  filter(ClaimType == 1))$ClaimCost), freq = FALSE, breaks = 50, xlab = "Claim Cost", main = "Histogram of log(claim cost) \nfor branch 1")

hist((data %>% 
  filter(ClaimType == 1))$ClaimCost, freq = FALSE, xlim = c(0, 2*10^5), breaks = 100, xlab = "Claim Cost", main = "Zoomed histogram of claim cost \nfor branch 1")
